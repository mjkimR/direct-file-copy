...
- **Broken symlinks are filtered out** — resolved (2026-07): Switched the `fs.existsSync` filter in `extension.ts` to `fs.lstatSync` check, allowing broken symlinks themselves to be copied by the native backends.
