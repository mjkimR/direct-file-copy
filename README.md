...
- **Clean Notifications**: Shows the copied file name (for single items) or a clean count like `Copied 3 items as file object(s).` (for multiple items) to prevent notifications from getting truncated.
- **Cross-Platform Support**:
  ...
  - **WSL (Windows Subsystem for Linux)**: Automatically converts Linux-style WSL paths (e.g. `/home/user/...` or `/mnt/c/...`) into Windows-compatible paths and UNC paths (using `wslpath`) and writes them directly to the Windows host clipboard using the host's PowerShell.
